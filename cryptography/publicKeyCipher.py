...
def main():
    filename = 'encryted_file.txt'  # The file to write or to read from
...
def getBlocksFromText(message, blockSize):
    for character in message:
        if character not in message:
            sys.exit('Error: The symbol set does not have the character %s'%(character))
    blockInts = []
    ...
    for blockInt in blockInts:
        ...
        for i in range(blockSize - 1, -1, -1):
            if len(message) + i < messageLength:
                ...
                blockMessage.append(SYMBOLS[charIndex])
        blockMessage.reverse()
...
def encryptMessage(message, key, blockSize):
    ...
    for block in getBlocksFromText(message, blockSize):
        encryptedBlocks.append(pow(block, e, n))    # ciphertext = plaintext ^ e mod n
...
def decryptMessage(encryptedBlocks, messageLength, key, blockSize):
    ...
    for block in encryptedBlocks:
        decryptedBlocks.append(pow(block, d, n))    # plaintext = ciphertext ^ d mod n
