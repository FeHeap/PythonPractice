...
def kasiskiExamination(ciphertext):
    ...
    allLikelyKeyLength = []
    for twoIntTouple in factorsByCount:
        allLikelyKeyLength.append(twoIntTouple[0])
...
def attemprHackWithKeyLength(ciphertext, mostlikelyKeyLength):
    ...
    for indexes in itertools.product(range(NUM_MOST_FREQ_LETTERS), repeat=mostlikelyKeyLength):
        ...
        if not SILENT_MODE:
            print('Attepting with key: %s'%(possibleKey))
...
def hackVigenere(ciphertext):
    ...
    for keyLength in allLikelyKeyLengths:
        ...
        hackedMessage = attemprHackWithKeyLength(ciphertext, keyLength)
        if hackedMessage != None:
            break
